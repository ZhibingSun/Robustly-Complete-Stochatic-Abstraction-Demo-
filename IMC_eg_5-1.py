...
dirname = '/Users/z235sun/Desktop/IMC_eg_5-1'
...
def main():
    ...
    with open(dirname + '/imc71.txt'.format(1), 'w') as f:
        f.write(f"{imc.n_matrix}\n")
        # for i, q in enumerate(imc.index_cube[:-1]):
        for i, q in tqdm(enumerate(imc.idx_cube[:-1]),total = imc.n):
...
with open(dirname + '/label71_1.txt', 'w') as f:
    labels = imc.compute_label(targets)
    for label in labels:
        print(len(label)-1)
        for i in range(len(label)-1):
            f.write(f"{label[i]} ")
    f.write(f"{imc.n_matrix} {imc.m}\n")
    # f.write(f"{imc.n >> 1} {imc.n_matrix} {imc.m}\n")
